  ...
  const runSimulation = useCallback((codeInput: string) => {
    ...
    lines.forEach((line, lineIdx) => {
      ...
      if (appendMatch) {
         ...
         newSteps.push({
            id: `step-${lineIdx}`,
            description: `arr.append(${pushVal}) - Add to end`,
      ...
      if (trimmed.includes('arr.pop()')) {
         ...
         newSteps.push({
            id: `step-${lineIdx}`,
            description: `arr.pop() - Removed ${val ?? 'nothing'}`,
            data: toLinearItems([...simulatedArr]),
            activeLine: lineIdx + 1
         });
         return;
      ...
      // 5. Insert: arr.insert(i, x)
      const insertMatch = trimmed.match(/arr\.insert\(\s*(\d+)\s*,\s*(\d+)\s*\)/);
      if (insertMatch) {
          const idx = parseInt(insertMatch[1]);
          const val = parseInt(insertMatch[2]);
          simulatedArr.splice(idx, 0, val);
          newSteps.push({
            id: `step-${lineIdx}`,
            description: `arr.insert(${idx}, ${val}) - O(N) Shift`,
            data: toLinearItems([...simulatedArr], idx),
            activeLine: lineIdx + 1,
            highlightedIndices: Array.from({length: simulatedArr.length - idx}, (_, i) => i + idx) // Highlight shifted part
         });
         return;
      }
    });

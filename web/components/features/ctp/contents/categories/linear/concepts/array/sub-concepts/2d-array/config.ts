...
export const ARRAY_2D_CONFIG: CTPModuleConfig = {
...
그래프 탐색(BFS/DFS), DP 등 수많은 알고리즘 문제의 기본이 되는 구조입니다.`,
    definition: `데이터를 '행(Row)'과 '열(Column)'의 2차원 표(Grid) 형태로 관리하는 자료구조입니다.

**불변식 (Invariants)**
- 0 ≤ r < R, 0 ≤ c < C 범위를 항상 지킨다.
- \`grid[r][c]\`는 (r, c) 좌표의 유일한 값이다.

**Graph List(인접 리스트)와 비교**
- 장점: 두 점의 연결 여부를 O(1)에 확인 가능, 구현이 직관적.
- 단점: 희소 행렬(대부분이 0)일 때 메모리 낭비 심함(N^2).`,
    analogy: `엑셀(Excel) 시트나 바둑판을 떠올리면 가장 정확합니다. 우리는 (행, 열) 두 개의 숫자로 정확한 위치를 집어낼 수 있습니다.

**실생활 예시**
- **이미지 처리**: 사진은 픽셀들의 2차원 배열입니다. (RGB는 3차원)
- **게임 맵**: 체스판, 지뢰찾기, RPG 게임의 타일 맵은 모두 2차원 배열입니다.
- **그래프 표현**: 정점들 간의 연결 관계를 '인접 행렬'로 표현합니다.`,
    playgroundLimit: "작은 2차원 배열을 만들고, (r, c) 좌표를 통해 값을 수정해보세요. 드래그로 범위를 선택해서 채워볼 수도 있습니다.",
    playgroundDescription: `이번 단계에서 무엇을 볼까?
- (r, c) 좌표 접근 패턴
- 행 우선 순회와 세로 순회의 성능 차이
- 방향 벡터(dr/dc)로 인접 칸을 탐색하는 방법

**실습 요약**
- 좌표 경계 체크가 모든 격자 문제의 기본임을 확인
- 행 우선 순회의 효율성을 체감`
  },

...
export function ChannelSidebar({ projectId }: ChannelSidebarProps) {
  const { channels, currentChannelId, joinChannel, createChannel } = useSocketStore();
  ...
  const handleCreate = () => {
    ...
    createChannel(projectId, newChannelName, newChannelDesc, "u1");
    setNewChannelName("");
    setNewChannelDesc("");
  ...
  return (
    <div className="flex flex-col h-full bg-muted/10 w-64 border-r">
      ...
      <div className="px-3 flex items-center justify-between group cursor-pointer mb-2 mt-4">
        <span className="font-semibold text-xs text-muted-foreground uppercase tracking-wider">Channels</span>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
                <div onClick={(e) => e.stopPropagation()} role="button" className="opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-muted/50 rounded">
      ...
      <ScrollArea className="flex-1">
        <div className="px-2 space-y-0.5">
          {channels.map((channel) => (
            ...
            >
              {channel.type === 'public' ? <Hash className="h-4 w-4 opacity-70" /> : <Volume2 className="h-4 w-4 opacity-70" />}
